...
<div class="offcanvas offcanvas-end" tabindex="-1" id="memberOffcanvas" aria-labelledby="memberOffcanvasLabel">
	...
	<div class="offcanvas-body pt-0">
		<?php if($is_member) { ?>
				...
				<div class="flex-grow-1 ps-1">
					<div class="d-flex gap-1">
						...
						<?php if ($is_admin === 'super') { ?>
							<div class="ms-auto">
								<a href="#adminOffcanvas" data-bs-toggle="offcanvas" data-bs-target="#adminOffcanvas" aria-controls="adminOffcanvas" class="btn btn-basic btn-sm" title="어드민">
			...
			<?php if(IS_EXP) { ?>
				...
				<div class="mb-2" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Next <?php echo number_format($member['as_max'] - $member['as_exp']) ?>">
					<div class="progress" role="progressbar" aria-label="Exp" aria-valuenow="<?php echo $member['as_per'] ?>" aria-valuemin="0" aria-valuemax="100">
						<div class="progress-bar progress-bar-striped progress-bar-animated small" style="width: <?php echo $member['as_per'] ?>%"><?php echo $member['as_per'] ?>%</div>
					</div>
				</div>
			<?php } ?>
			...
			<a href="<?php echo G5_BBS_URL ?>/logout.php" class="btn btn-primary btn-lg w-100 mb-2" role="button">
		...
		<?php } else { ?>
			...
			<form id="memberLogin" name="memberLogin" method="post" action="<?php echo G5_HTTPS_BBS_URL ?>/login_check.php" autocomplete="off">
