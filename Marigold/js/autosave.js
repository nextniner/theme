	...
	$("#btn_autosave").click(function(){
		...
		$.get(g5_bbs_url+"/ajax.autosavelist.php", function(data){
			...
			if ($(data).find("list").find("item").length > 0) {
				$("#naClipView").html('<div id="autosave_pop"><ul class="list-group list-group-flush border-top border-bottom"></ul></div>');
				...
				$("#naClipView").html('<div id="autosave_pop"><ul class="list-group list-group-flush border-top border-bottom"><li class="list-group-item">등록된 임시저장글이 없습니다.</li></ul></div>');
			}
		}, "xml");
