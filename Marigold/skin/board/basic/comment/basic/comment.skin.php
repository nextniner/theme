		...
		<article id="c_<?php echo $comment_id ?>" <?php if ($comment_depth) { ?>style="margin-left:<?php echo $comment_depth ?>rem;"<?php } ?>>
			<div class="comment-list-wrap position-relative">
				<header style="z-index:<?php echo $comment_sv ?>">
					...
					<div class="d-flex align-items-center border-top <?php echo $by_writer ?> py-2 px-3 small">
...
<?php if ($is_comment_write) { ?>
	...
	function fviewcomment_submit(f) {
		...
		if (typeof(f.wr_name) != 'undefined') {
			f.wr_name.value = f.wr_name.value.trim();
			if (f.wr_name.value == '') {
		...
		if (typeof(f.wr_password) != 'undefined') {
			f.wr_password.value = f.wr_password.value.trim();
		...
		// 양쪽 공백 없애기
		document.getElementById('wr_content').value = document.getElementById('wr_content').value.trim();
