...
if ($is_category) {
	...
	// 분류 가로수
	$cols = array();
	$cols[] = (isset($boset['cate_de']) && (int)$boset['cate_de'] > 0) ? 'row-cols-'.$boset['cate_de'] : 'row-cols-3';
	$cols[] = (isset($boset['cate_sm']) && (int)$boset['cate_sm'] > 0) ? 'sm-'.$boset['cate_sm'] : 'sm-3';
	$cols[] = (isset($boset['cate_md']) && (int)$boset['cate_md'] > 0) ? 'md-'.$boset['cate_md'] : 'md-4';
	$cols[] = (isset($boset['cate_lg']) && (int)$boset['cate_lg'] > 0) ? 'lg-'.$boset['cate_lg'] : 'lg-5';
	$cols[] = (isset($boset['cate_xl']) && (int)$boset['cate_xl'] > 0) ? 'xl-'.$boset['cate_xl'] : 'xl-5';
	$cols[] = (isset($boset['cate_xxl']) && (int)$boset['cate_xxl'] > 0) ? 'xxl-'.$boset['cate_xxl'] : 'xxl-6';
	$cate_cols = implode(' row-cols-', $cols);
?>
	<nav id="bo_category" class="border-top py-2">
		<ul class="nav row <?php echo $cate_cols ?> g-1 small">
